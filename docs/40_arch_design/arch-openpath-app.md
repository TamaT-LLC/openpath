...
- 保護フォルダ（NFR-02 の例外）: `roots` にホーム（`~`）など `~/Desktop`・`~/Documents`・`~/Downloads` を含む場所、またはその中を指定した場合（ghq の root が取れないときの既定 `roots = ["~"]` を含む）に限り、macOS（TCC）のアクセス確認が出ることを許容する。Full Disk Access は要求しない。
  - 確認に理由を出すため、`Resources/Info.plist` に `NSDesktopFolderUsageDescription`・`NSDocumentsFolderUsageDescription`・`NSDownloadsFolderUsageDescription` を置く。説明は日本語のみ（`CFBundleDevelopmentRegion` は `ja`。`scripts/build.sh` は `*.lproj` をバンドルにコピーしない）。
  - 許可・拒否は、あとから「システム設定 > プライバシーとセキュリティ > ファイルとフォルダ」で変えられる。
  - 想定される挙動（実装と macOS の仕様からの想定で、実機では未確認。PROJ-TST-002 の ONB-20 で確認予定で、結果に応じて更新する）:
    - 確認は候補の走査で初めてその中を読むときに出て、本文に上記の用途の説明が表示される。
    - ホームなど保護フォルダを含む場所を走査している場合は、拒否されても走査は続く（読めない下位ディレクトリは飛ばす）。拒否したフォルダ自体は候補に残り、中身が候補に入らないだけになる。
    - 保護フォルダ自体を `roots` に指定して拒否した場合は、そのルートの中身が候補に入らない。ルートを読めないと判定すればルート自体も候補から外し、「候補のルートを走査できませんでした」の警告ログを出す。TCC の拒否でこの判定になるかも、あわせて確かめる。
