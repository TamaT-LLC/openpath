...
- 保護フォルダ（NFR-02 の例外）: `roots` にホーム（`~`）など `~/Desktop`・`~/Documents`・`~/Downloads` を含む場所、またはその中を指定した場合（ghq の root が取れないときの既定 `roots = ["~"]` を含む）、候補の走査で初めてその中を読むときに macOS（TCC）がアクセスの確認を出す。
  ...
  - 保護フォルダ自体を `roots` に指定して拒否した場合は、そのルートの中身が候補に入らない。ルートを読めないと判定すればルート自体も候補から外し、「候補のルートを走査できませんでした」の警告ログを出す。TCC の拒否でこの判定になるかは未確認。
  - どちらの場合も、あとから「システム設定 > プライバシーとセキュリティ > ファイルとフォルダ」で変えられる。
