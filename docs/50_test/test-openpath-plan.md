...
- 手順:
  ...
  5. osascript を終了してダイアログを閉じ、`panel gone` が出るまで待つ（5 秒）。openpath は同時に 1 つのパネルだけを追跡し、次の `panel detected` の前に必ず `panel gone` を出すため、受け入れた `panel detected` の後の最初の `panel gone` をこのダイアログのものとみなす。
