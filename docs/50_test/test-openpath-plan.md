...
| S-07a | 任意 | 漢字名のディレクトリ `~/Documents/資料` を選択 | 正しく移動する（日本語を含むパスの注入、REQ-001 §5 受け入れ基準） |
| S-07b | 任意 | カタカナ名のディレクトリ（例 `シリョウ`）をひらがな（`しりょう`）で検索する | 候補に出て移動できる（かな同一視、DSN-002 §5） |
...
- S-07b: 漢字の読みでの検索（`しりょう` で `資料` を検索すること）は現仕様の対象外。Phase 2 の機能として Issue #87 で扱う（#86 の QA 結果を受けたオーナー判断、2026-09-26）。
