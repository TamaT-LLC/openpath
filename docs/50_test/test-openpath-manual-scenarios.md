...
| ID | 手順 | 期待 | 結果 | 備考 |
| --- | --- | --- | --- | --- |
| ONB-01 | `open build/openpath.app` | `lsappinfo info -only type -app jp.tamat.openpath` が `"type"="UIElement"`（Dock に出ない）。ログに `openpath 0.1.0 を起動します` → `起動処理を終えました（アクセシビリティ権限: なし）` → `ホットキー ctrl+shift+o を登録しました` → `初回起動の案内: notShown → explainingPermission` |  |  |
...
| ONB-09 | ダイアログを見る | 300ms 以内にパレットが重なり、ディレクトリだけが候補に出る（ログ `panel detected (… directoriesOnly: true)`）。前面に出なかった場合は、ログに `前面に出せませんでした` / `登録されませんでした` が無いか、ダイアログをクリックするとパレットが出るかを備考に書く |  |  |
...
| SMK-02 | `./scripts/smoke-open-panel.sh`（ダイアログが最前面に出なければクリックする） | 最後の行が `OK`、終了コード 0。`panel detected` の時刻と、`palette shown` までの時間（ログにあれば）を備考に写す |  |  |
...
| ID | 手順 | 期待 | 結果 | 備考 |
| --- | --- | --- | --- | --- |
| S-01 | TextEdit の「ファイル > 開く…」（⌘O、§1.1） | 300ms 以内にパレットがパネルの右上（内側 12pt）に出る（ログの `panel detected` の時刻）。検索フィールドにフォーカスがあり、打った文字がパレットに入る。空入力では frecency の上位（履歴が無ければパス順）が出る |  |  |
...
| PAL-06 | 一致しない語を打つ | リストに「一致する候補がありません。Tab でパスを直接入力」が出る |  |  |
...
| MENU-02 | 「有効」を外したまま終了して起動し直す → 付け直して起動し直す | 無効のまま起動する（`パネルの監視を始めました` とホットキーの登録が無い）。付け直した後は有効で起動する（§1.2。修正 PR のマージ後） |  |  |
