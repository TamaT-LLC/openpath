...
- NFR-02: アクセシビリティ権限のみを要求し、Full Disk Access は要求しないこと。
  - 例外（2026-09-24 オーナー判断）: `roots` にホーム（`~`）など macOS の保護フォルダ（デスクトップ・書類・ダウンロード）を含む場所、または保護フォルダの中を指定した場合（ghq の root が取れないときの既定 `roots = ["~"]` を含む）に限り、保護フォルダへのアクセス確認が出ることを許容する。確認に理由を出すため、Info.plist に用途の説明を置く。
  - 想定される挙動（実機では未確認。PROJ-TST-002 の ONB-20 で確認予定で、結果に応じて更新する）: 確認は候補の走査で初めてその中を読むときに出て、用途の説明が表示される。拒否しても動作は継続し、そのフォルダの中身が候補に入らないだけになる。ただし保護フォルダ自体を `roots` に指定した場合は、そのルートを読めないと判定すれば、ルート自体も候補に入らない（ARCH-001 §9）。
