...
#   --start-pattern TEXT 始点の行の目印。既定 "panel detected"
#   --end-pattern TEXT   終点の行の目印。既定 "palette shown"（パレットを表示したときに "panel detected" と同じく
#                        info で出る文言。これを出さない古いビルドのログでは end の行が無く、終了コード 2 になる）
#   --threshold-ms MS    合格とする p95 の上限（ミリ秒、これ以下なら PASS）。既定 300
#
# 計測の手順: アクセシビリティ権限を付けた openpath を起動し、Finder で Cmd+O → パレットが出たのを確かめて
# パネルを「キャンセル」で閉じる操作（TST-001 §3 の S-01）を 20 回ほど繰り返してから、このスクリプトを実行する。
#
...
# 組み方: start の行の後、次の start または "panel gone" の行より前に最初に現れた end の行と組にする。
#   組めなかった start は件数だけ数える。直前に start の無い end は組にしない。
...
# "summary <start 行数> <end 行数> <組めなかった start> <start の無い end> <負の差> <解釈できないタイムスタンプ>" を出力する。
# 目印は -v だとエスケープが解釈されるため環境変数（START_PATTERN / END_PATTERN / GONE_PATTERN）で受け取る
...
main() {
  ...
  pairing="$(START_PATTERN="${start_pattern}" END_PATTERN="${end_pattern}" GONE_PATTERN="${PANEL_GONE_PATTERN}" \
    awk "${PAIRING_AWK}" "${log_files[@]}")" || die_unmeasurable "ログを集計できません"
  local summary
  ...
  printf '始点: "%s" / 終点: "%s"（間に "%s" があれば組にしない）\n' "${start_pattern}" "${end_pattern}" "${PANEL_GONE_PATTERN}"
  printf 'start の行: %s 件、end の行: %s 件\n' "${starts}" "${ends}"
  if [[ "${bad_timestamps}" -gt 0 ]]; then
    warn "タイムスタンプを解釈できない行を ${bad_timestamps} 行飛ばしました"
  fi
  if [[ "${orphan_ends}" -gt 0 ]]; then
    warn "直前に start の無い end の行が ${orphan_ends} 行ありました（組にしていません）"
  fi
  ...
  if [[ "${ends}" -eq 0 ]]; then
    die_unmeasurable "終点の行（\"${end_pattern}\"）がログに 1 行も無いため計測できません。パレット表示時に \"${DEFAULT_END_PATTERN}\" を info で出すビルドで計測し直すか、終点の文言が違う場合は --end-pattern で指定してください"
  fi
  ...
  printf '最大: %s ms\n' "${maximum}"
  printf '組めなかった start: %s 件（次の start・"%s"・ログの末尾までに end が無かった）\n' "${unpaired}" "${PANEL_GONE_PATTERN}"
