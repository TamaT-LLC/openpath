...
- 設定ファイル `~/.config/openpath/config.toml` が無ければ、既定の内容で作ります（既存のファイルは上書きしません）。候補として走査するディレクトリ `roots` は、ghq があれば `ghq root`、無ければホーム（`~`）になります。ホームにした場合、`~/Desktop`・`~/Documents`・`~/Downloads` の中を初めて読むときに macOS がアクセスの許可を確認することがあります。
